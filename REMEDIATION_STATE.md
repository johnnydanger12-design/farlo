...
4. **Next up:** UI/UX's remaining full-app `Semantics` coverage (116 Dart files per `ux-review.md`'s own framing — a much larger effort than either pass attempted so far) and Security's GDPR data-export mechanism (bigger, product-shaped — needs scoping before it's a Fix-Protocol item) are the two largest remaining gaps toward A+ in any category.
5. Before claiming any category A+: a full (non-sampled) re-verification pass at ≥97, per the operating prompt — not yet done for any category.
