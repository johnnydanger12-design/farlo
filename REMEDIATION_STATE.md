...
|---|---|---|---|---|---|
| **Overall** (weighted, Product excluded) | ~85 | **~93** | ≥97 | — | ~4 pts |
| Security | 86 | **~95** | zero Medium+ findings + permanent tests | 25% | GDPR export now built; Low findings not required but noted |
...
**No category has reached the ≥97 A+ threshold yet.** Per the operating prompt, A+ cannot be claimed for any category without a full (non-sampled) re-verification pass at ≥97 — none qualify for that check yet, so none are claimed. No single large gap remains identified in any category — the next step toward any A+ claim is that full re-verification pass itself, not a specific known-missing feature.
...
6. **Next up:** no single large remaining gap identified — before claiming any category A+, a full (non-sampled) re-verification pass at ≥97 is required per the operating prompt, not yet done for any category. Remaining smaller items: Low-severity security findings (not required for A+ but noted), the `brand` storage bucket's missing size/MIME limit (noted during ARCH-5 research, not yet closed).
